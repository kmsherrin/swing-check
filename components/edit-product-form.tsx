...
import { getCategories } from "@/app/(account)/account/restaurant/[restaurant_id]/products/new/fetching";
import { getProduct } from "@/app/(account)/account/restaurant/[restaurant_id]/products/fetching";
import { updateProduct } from "@/app/(account)/account/restaurant/[restaurant_id]/products/[product_id]/edit/actions";

const formatProductOptionsToDisplay = (
  productData: Awaited<ReturnType<typeof getProduct>>
) => {
  const productOptions = productData.product.options;
  ...
  optionKeys.forEach((key, index) => {
    const option = productOptions[key] as {
      name: string;
      type: string;
      values: { [key: string]: { price: string; value: string } };
    };
    ...
    const returnStructure = {
      ...
      name: option.name,
      type: option.type,
      numberOfOptions: Object.keys(option.values).length,
    };

    optionsArray.push(returnStructure);
  });
...
export const EditProduct = ({
  ...
  restaurantId: string;
  productId: string;
  existingCategories: Awaited<ReturnType<typeof getCategories>>;
  existingProduct: Awaited<ReturnType<typeof getProduct>>;
}) => {
  const [productOptions, setProductOptions] = useState(
    formatProductOptionsToDisplay(existingProduct)
  );

  const [message, formAction] = useFormState(updateProduct, null);
    ...
    <>
      <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-1 lg:gap-8 max-w-screen-md">
        ...
        >
                ...
                <div className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Input
                      ...
                      type="text"
                      className="w-full"
                      defaultValue={existingProduct.product.title}
                      required
                    />
                  </div>
                  <div className="grid gap-3">
                    ...
                    <Input
                      ...
                      className="w-full"
                      defaultValue={
                        existingProduct.product.shortDescription || ""
                      }
                      required
                    />
                  </div>
                  <div className="grid gap-3">
                    ...
                    <Textarea
                      name="description"
                      id="description"
                      defaultValue={existingProduct.product.description || ""}
                      className="min-h-32"
                    />
                  </div>
                  <div className="grid gap-3">
                    ...
                    <Input
                      ...
                      name="basePrice"
                      type="number"
                      defaultValue={existingProduct.product.basePrice}
                      id="productBasePrice"
                      min="0.00"
                      max="10000.00"
                      step="0.01"
                  ...
                  <div className="grid gap-3">
                    ...
                    {existingProduct.product.image && !editingImage ? (
                      <div className="relative">
                        <Image
                          ...
                          height={300}
                          alt={"Product Image"}
                          src={`${process.env.NEXT_PUBLIC_SUPABASE_IMAGE_URL}/restaurant-images/restaurant-${restaurantId}/${existingProduct.product.image}.webp?width=30&height=30`}
                        />
                        <Button
                          type="button"
                          variant={"destructive"}
              ...
              <CardContent>
                <div className="grid gap-4">
                  {productOptions.map((option, index) => (
                    <div
                      key={index}
                      className={`relative grid gap-3 border-b-2 pb-6 pt-6`}
                    >
                      <Button
                        type="button"
                        onClick={() =>
                          setProductOptions(
                            productOptions.filter((_, i) => i !== index)
                          )
                        }
                        variant={"ghost"}
                        className={`absolute top-0 right-0 hover:bg-red-50`}
                      >
                        <X className="w-4" />
                      </Button>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div>
                          ...
                          <Input
                            ...
                            className="w-full"
                            defaultValue={
                              existingProduct.product.options[index]?.name || ""
                            }
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            defaultValue={
                              existingProduct.product.options[index]?.type ||
                              "single-select"
                            }
                            onValueChange={(val) => {
                              ...
                              >
                                <div className="flex gap-2 items-center">
                                  ...
                                  <Input
                                    ...
                                    defaultValue={
                                      existingProduct.product.options[index]
                                        ?.values[i]?.value || ""
                                    }
                                  />
                                </div>
                ...
                <div className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Select
                      name="category"
                      required
                      defaultValue={existingProduct.product_category?.id}
                    >
                ...
                <div className="grid gap-6">
                  <div className="grid gap-3">
                    ...
                    <Select
                      name="productStatus"
                      required
                      defaultValue={existingProduct.product.status}
                    >
                      <SelectTrigger
                        id="productStatus"
          ...
          <SubmitButton
            normalElement={
              <span className="flex gap-2 items-center">Update Product</span>
            }
            pendingElement={
              <span className="flex gap-2 items-center">
                <Loader2 className="animate-spin w-4" /> Updating Product...
              </span>
            }
          />
          {
          ...
          {/* {
              ...
              message?.success && (
                <div>
                  <h3 className="text-green-500  tracking-tight">
                    {message.success}
                  </h3>
                </div>
              )
